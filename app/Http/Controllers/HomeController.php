...
class HomeController extends Controller
{
    ...
    public function search($search_content){
        ...
        // $search_result = DB::select('select * from projects natural join project_tag join tags on tags.id = project_tag.tag_id
        //                             where pname like ? or desp like ? or tags.name like ? limit 10', ['%' . $search_content . '%', '%' . $search_content . '%', '%' . $search_content . '%']);

        foreach($search_result as $result){
            app('App\Http\Controllers\UserLogController')->store($result->pid,2);
        }
        return view('search/index', ['search_result' => $search_result]);
